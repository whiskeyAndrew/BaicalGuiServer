...
//Обработчик пакетов. Каждый пакет в библиотеке содержит в себе заголовок пакета и чанки внутри него.
//Определяет тип пакетов и либо сам обрабатывает их, либо отправляет их в chunkHandler, если это пакет с телеметрией или трейсами
//генерирует сам ответы на пинг пакеты чтобы поддерживать актуальное соединение с клиентом
