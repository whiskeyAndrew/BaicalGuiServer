...
//принимает чанки из packetHandler (или из fileReader если это файл) и обрабатывает каждый чанк с данными внутри.
//Чанки состоят из подчанков с данными по трейсам или телеметрии
//В этом приложении вся телеметрия отправляется в мусорку, задача была обрабатывать и выводить конкретно трейсы
//Данные о новых модулях, новых трейсах и любых неновых трейсах отправляется в traceWindow, который отвечает за отображение и вывод по заданным фильтрам
